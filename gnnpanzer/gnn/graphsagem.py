...
class GraphSAGEModel(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: Union[int, Arrays], num_layers: int, out_channels: int,
        ...
        # input
        self.convs.append(SAGEConv(in_channels, hidden_channels[0], bias=with_bias, normalize=normalize,
                                   aggr=conv_aggr, root_weight=conv_root_weight, project=conv_project))
        if with_bn:
        ...
        for layer_i in range(num_layers-2):
            self.convs.append(SAGEConv(int(hidden_channels[layer_i]), hidden_channels[layer_i+1],
                                       bias=with_bias, normalize=normalize,
                                       aggr=conv_aggr, root_weight=conv_root_weight, project=conv_project))
            if with_bn:
        ...
        self.convs.append(SAGEConv(int(hidden_channels[-1]), out_channels, bias=with_bias, normalize=normalize,
                                   aggr=conv_aggr, root_weight=conv_root_weight, project=conv_project))
        # single layer
